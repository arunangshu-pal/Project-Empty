...
<pre>   $ git clone https://github.com/arunangshu-pal/Project-Empty </pre>
   
2. Run the "install.sh" bash script placed inside the "Empty" directory as follows. <br/>
<pre>   $ cd Empty
   $ sudo chmod +x install.sh    # to make install.sh executable
   $ sudo ./install.sh           # install.sh compiles the source file to create a binary called "empty", and places a symbolic link to the binary in /usr/local/bin directory </pre>

3. Installation of the command "empty" is complete. Enjoy!

### Usage: <br/>
The binary is named "empty". Enter the following in a bash shell to see the help description. <br/>
<pre> $ empty --help </pre>
...
For the list of options [OPTION], the following options are available: <br/>
-f, --force
